...
`index.html` (Buscar Peça), `demanda.html` (Demanda Cliente), `expedicao.html` (Expedição), `kanban.html`, `op-kanban.html` (Painel), `cliente.html` (Consulta Cliente), `apontamentos.html`, `prazos.html`, `otd.html`, `ordens.html`, `paradas.html`, `operadores.html` (Performance), `qualidade.html` (Qualidade), `cadastro-item.html` (Cadastro Item), `engenharia.html` (Cadastro Roteiro — roteiros de processo).

O menu de navegação (`.nav` no topbar) deve conter as 15 páginas, em **todas** as páginas, na mesma ordem (a sequência acima). Reordenar/inserir via script Node que reescreve o bloco `.nav` (e `.mobile-nav` onde existir).
...
- **Cadastro** (grupo de menu "Cadastro ▾" → Item / Roteiro): reestruturação de 07/2026 — separa quem cadastra a peça (Comercial) de quem monta o roteiro (Engenharia). **Não substitui o fluxo do Forms** — o fluxo atual (Google Forms pré-preenchido → "super base" no Google Sheets → sincroniza em `producao_eventos`) é mantido como está; o Cadastro Item é um registro **paralelo**, próprio da Engenharia, para peças que ainda vão ganhar roteiro.
