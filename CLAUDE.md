...
- **Engenharia** (`engenharia.html`): cadastro de roteiros de processo (spec em `docs/modulos/roteiro-processo.md`). **Não substitui o cadastro do item** — o fluxo atual (Google Forms pré-preenchido → "super base" no Google Sheets → sincroniza em `producao_eventos`) é mantido como está. Dois jeitos de criar roteiro, **cada um com item próprio no menu** (grupo "Engenharia ▾" → "Item existente" / "Item novo", ambos apontam pro mesmo arquivo com `?modo=novo` distinguindo — ver nota abaixo sobre `atualizar_nav.js`): **peça existente** (busca por código/pedido/OS em `posicao_atual`, cabeçalho já vem preenchido) ou **peça nova** (painel dedicado pra digitar código/pedido/OS/qtd na mão, pra planejar o roteiro antes da peça passar pelo Forms; fica com aviso "⚠ peça nova" na lista/editor até constar em `posicao_atual`). Um roteiro por **pedido+código** (`roteiros`); paleta de operações do catálogo (`roteiro_operacoes`) agrupada por família reordenável (`roteiro_familias`) — um clique adiciona na sequência (`roteiro_itens`); subcomponente por linha (lista em `roteiro_subcomponentes`, chips definem o das novas linhas); tudo com salvamento imediato/debounce. ⚙ abre manutenção do catálogo e subcomponentes. 🖨 gera impresso (layout tipo Excel, agrupa por subcomponente) com QR Code do **link do Forms pré-preenchido montado automaticamente** (`montarLinkForms` no engenharia.html: base + `FORM_ENTRY.*` do Forms "PILOTO 2 REV.A"; preenche razão social, classif. fiscal, código, pedido, OS, qtd solicitada, **qtd de operações = nº de linhas do roteiro**, cliente e prazo). Peça existente puxa cliente/tipo/prazo de `producao_eventos`; peça nova pede Tipo (V→Brasmat/Vendas, I→Mota/Industrialização) + Cliente (lista `roteiro_clientes`) + Prazo no painel "Item novo". Campo `roteiros.forms_link` continua como **override manual opcional**. Se o Forms for recriado, reconferir os `entry.ID` em `FORM_ENTRY`; a lista de clientes é re-sincronizada do HTML público do Forms sob demanda. Quando a peça tem roteiro, a linha do tempo usa o **total de operações do roteiro** como denominador do progresso (`buscarTotalRoteiro` em status.js; fallback = comportamento antigo).
