...
- **Engenharia** (`engenharia.html`): cadastro de roteiros de processo (spec em `docs/modulos/roteiro-processo.md`). Busca peça por código/pedido/OS → cria/abre roteiro (um por **pedido+código**, `roteiros`); paleta de operações do catálogo (`roteiro_operacoes`) agrupada por família — um clique adiciona na sequência (`roteiro_itens`); subcomponente por linha (lista em `roteiro_subcomponentes`, chips definem o das novas linhas); tudo com salvamento imediato/debounce. ⚙ abre manutenção do catálogo e subcomponentes. 🖨 gera impresso (layout tipo Excel, agrupa por subcomponente) com QR Code apontando pra `engenharia.html?pedido=&codigo=` (deep-link abre o roteiro). Quando a peça tem roteiro, a linha do tempo usa o **total de operações do roteiro** como denominador do progresso (`buscarTotalRoteiro` em status.js; fallback = comportamento antigo).
